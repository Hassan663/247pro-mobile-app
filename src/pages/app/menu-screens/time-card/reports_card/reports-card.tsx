...
const ReportCard = ({navigation}) => {
  ...
  const currentUserProfile = useSelector(
    (state: any) => state.root.currentUserProfile,
  );
    ...
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        response.forEach((timesheet, index) => {
          const timeInMinutes = calculateTotalTime(
            timesheet.timesheetTransactions,
          );
          console.log(
            `Timesheet ${index + 1} -> Calculated Time (Minutes):`,
            timeInMinutes,
          );
  ...
  const calculateTotalTime = transactions => {
    ...
    if (transactions && transactions.length > 0) {
      ...
      if (transactions.length === 1) {
        ...
        const hasBreakOut = transactions.some(
          transaction => transaction.transactionType === 2,
        );

        if (!hasBreakOut) {
          const earliestTransaction = moment(
            transactions[0].transactionDateTime,
          );
          const currentTime = moment();
          const duration = moment.duration(
            currentTime.diff(earliestTransaction),
          );
          ...
          const duration = moment.duration(
            latestTransaction.diff(earliestTransaction),
          );
  ...
  const getTransactionDetails = transactionType => {
    switch (transactionType) {
      case 1:
        return {action: 'Clock in', color: '#FB9411'};
      case 2:
        return {action: 'Clock out', color: '#B00020'};
      case 3:
        return {action: 'Break in', color: '#2196F3'};
      case 4:
        return {action: 'Break out', color: '#4CAF50'};
      default:
        return {action: 'Unknown', color: 'gray'};
    }
  };

  const sortTransactionsByDate = transactions => {
    return transactions.sort(
      (a, b) => moment(a.transactionDateTime) - moment(b.transactionDateTime),
    );
  };
        ...
        <>
          ...
          <ScrollView contentContainerStyle={styles.container}>
            <View>
              ...
              <Text style={styles.totalHoursTextHardCoded}>
                Total working hours
              </Text>

              <View style={styles.rowContainer}>
                <Text style={styles.totalHoursText}>
                  {formatTotalWorkingTime(totalWorkingHours)}
                </Text>
                  ...
                  <View key={index} style={styles.timesheetCard}>
                    <View style={styles.timesheetHeader}>
                      <Text style={styles.dateText}>
                        {moment(timesheet.timesheetDate).format('MM/DD/YYYY')}
                      </Text>
                      <Text style={styles.dateText}>
                        {formatReportTransactionTime(totalTimeInMinutes)}
                      </Text>
                    </View>

                    {/* {sortedTransactions.map((transaction, idx) => {
                      const {action, color} = getTransactionDetails(
                        transaction.transactionType,
                      );
                      const isLastTransaction =
                        idx === sortedTransactions.length - 1;
                      const hideLine =
                        isLastTransaction && transaction.transactionType === 2;
                      ...
                      if (transaction.transactionType === 1) {
                        ...
                        if (
                          ...
                          moment(transaction.transactionDateTime).format(
                            ...
                            moment(nextTransaction.transactionDateTime).format(
                              'MM/DD/YYYY',
                            )
                        ) {
                      ...
                      if (transaction.transactionType === 2) {
                        ...
                        if (
                          ...
                          moment(transaction.transactionDateTime).format(
                            ...
                            moment(prevTransaction.transactionDateTime).format(
                              'MM/DD/YYYY',
                            )
                        ) {
...
{sortedTransactions.map((transaction, idx) => {
  ...
  // Default format: time only (hh:mm A)
  let formattedDateTime = moment(transaction.transactionDateTime).format(
    'hh:mm A'
  );

  // Sideline logic: Hide the vertical line for the last transaction
  const isLastTransaction = idx === sortedTransactions.length - 1;
  const hideLine = isLastTransaction && transaction.transactionType === 2;

  // Use the original timesheetTransactions to check for mismatched dates
  if (transaction.transactionType === 1) {
    // Clock-In: Check if corresponding Clock-Out is on a different day
    const correspondingClockOut = timesheet.timesheetTransactions.find(
      (t) => t.transactionType === 2 && moment(t.transactionDateTime).isAfter(transaction.transactionDateTime)
    );

    if (
      correspondingClockOut &&
      !moment(transaction.transactionDateTime).isSame(
        correspondingClockOut.transactionDateTime,
        'day'
      )
    ) {
      // Add date to Clock-In if Clock-Out is on a different day
      formattedDateTime = moment(transaction.transactionDateTime).format(
        'MM/DD/YYYY hh:mm A'
      );
    }
  }
  ...
  if (transaction.transactionType === 2) {
    // Clock-Out: Check if corresponding Clock-In is on a different day
    const correspondingClockIn = timesheet.timesheetTransactions.find(
      (t) => t.transactionType === 1 && moment(t.transactionDateTime).isBefore(transaction.transactionDateTime)
    );

    if (
      correspondingClockIn &&
      !moment(transaction.transactionDateTime).isSame(
        correspondingClockIn.transactionDateTime,
        'day'
      )
    ) {
      // Add date to Clock-Out if Clock-In is on a different day
      formattedDateTime = moment(transaction.transactionDateTime).format(
        'MM/DD/YYYY hh:mm A'
      );
    }
