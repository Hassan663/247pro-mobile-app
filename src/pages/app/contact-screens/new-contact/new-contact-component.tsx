    ...
    onPress={async () => {
      try {
        if (!isToastVisible) {
          ...
          if (isValid.success) {
            if (
              inputValues.contactTypeId == 1 ||
              inputValues.contactTypeId == 4
            )
              inputValues.contactSpecialities = [];
        ...
        <View style={styles.imageContainer}>
          {item.profile ? (
            ...
            <Img
              ...
              customStyle={styles.placeholderAvatar}
            />
          )}
        </View>
        ...
        {invitePeopleModal ? (
          <View style={centralStyle.row}>
            <TouchableOpacity
              ...
              activeOpacity={0.9}
              style={[
                styles.tabContainer(selectedTab),
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.9}
              style={[
                styles.tabContainer2(selectedTab),
                centralStyle.XAndYCenter,
              ]}>
              <Title
                type="Poppin-14"
          ...
          <AlphabetList
            ...
            renderCustomItem={item => (
              <CompanyList
                ...
                getCompany={(val: any) => getCompany && getCompany(val)}
                item={item}
              />
            )}
  ...
  return (
    <View
      style={[
        ...
        {height: ALPHABET_SIZE.HEADER_HEIGHT},
      ]}>
      <Title
        color={Colors.black}
        type="Poppin-14"
    ...
    <View
      ...
      <TouchableOpacity
        ...
        <Image
          resizeMode="contain"
          source={
            attechments.type == 'application/pdf'
              ? require('../../../../assets/app-images/pdfIcon.png')
              : {uri: attechments.uri}
          }
          style={[centralStyle.height100, centralStyle.width100]}
        />
...
export const renderComponentOfContactEmails = ({
  item,
  index,
  inputValues,
  handleInputChange,
    ...
    <View
      ...
      <View style={{flex: 7}}>
        <OutlinedTextInput
          val={item.email}
          onChange={text =>
            handleInputChange('contactEmails', text, 'email', index)
          }
          title={t('Email')}
          placeHolder={t('Email')}
        />
      ...
      {item?.email?.length > 0 && (
        <View style={[{flex: 2.3, marginHorizontal: RFPercentage(0.6)}]}>
          <OutlinedDropDown
            ...
            onselect={(value: string) =>
              handleInputChange('contactEmails', value, 'label', index)
            }
            DATA={EMAILLABELDATA}
...
export const RenderComponentOfPhone = ({
  item,
  index,
  inputValues,
  HandleCountrySelect,
      ...
      <View
        ...
        <View style={{flex: 8.9}}>
          <OutlinedTextInput
            val={item.phone}
            onChange={text =>
              handleInputChange('contactPhones', text, 'phone', index)
            }
            title={t('MobilePhone')}
        ...
        {condition ? (
          ...
          <TouchableOpacity
            onPress={() =>
              removePrevFieldOfPhone(index, setInputValues, inputValues)
            }
            style={[
              centralStyle.flex1,
              centralStyle.justifyContentCenter,
...
export const SepecialityModal = ({
  ...
  const getSpeciality = (obj: {specialtyId: number; specialtyName: string}) => {
    ...
    let alreadySelected = deepCopyisSelectedValues.findIndex(
      (val: any) => val.specialtyId === obj.specialtyId,
    );
...
const SpecialityRenderModal = ({
  ...
  useEffect(() => {
    if (selectedData.length > 0) {
      const isAlready = selectedData.findIndex(
        ({specialtyName}: {specialtyName: string}) =>
  ...
  return (
    <TouchableOpacity
      ...
      <Title type="Poppin-14" title={item.name} />
      <MaterialIcons
...
export const SpecialityTags = ({
  ...
  item: {specialtyName: string};
  index: number;
  removeSpeciality: any;
}) => {
  return (
    <View style={styles.specialitytags}>
      <Title type="Poppin-12" title={item.specialtyName} />
      <TouchableOpacity onPress={() => removeSpeciality(index)}>
        <Entypo
          name={`cross`}
          style={styles.downIcon}
          color={Colors.fontColor}
