...
export const contactTypefilter = async (
  id: number,
  dispatch: any,
  setpageIndex?: any,
  pageIndex?: number,
  ...
  try {
    if (id !== 0) {
      await dispatch(TypeContactAction(id, setpageIndex, pageIndex));
    }
  } catch (error) {
...
export const getProContacts = async (
  dispatch: any,
  type: number,
  specialityID: number,
) => {
  try {
    if (type !== 0 && specialityID) {
      await dispatch(GetTypeContactsSpecialityAction(type, specialityID));
    }
  } catch (error) {
    console.log('error--->', error);
  }
